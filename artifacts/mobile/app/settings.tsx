  ...
  return (
    <View style={sectionStyles.root}>
      <View style={sectionStyles.header}>
        ...
        <Text style={[sectionStyles.title, { color: colors.primary }]}>
          {title.toUpperCase()}
        </Text>
      </View>
      <View
...
function SettingRow({
  ...
  const content = (
    ...
    >
      <View
        style={[rowStyles.iconWrap, { backgroundColor: colors.primary + "18" }]}
      >
      ...
      <View style={rowStyles.text}>
        ...
        {description && (
          <Text style={[rowStyles.desc, { color: colors.mutedForeground }]}>
            {description}
          </Text>
        )}
      </View>
      ...
      {options.map((opt, i) => (
        ...
        >
          ...
          >
            {value === opt.key && (
              <View
                style={[radioStyles.dot, { backgroundColor: colors.primary }]}
              />
            )}
          </View>
          <View style={radioStyles.text}>
            <Text style={[radioStyles.label, { color: colors.foreground }]}>
              {opt.label}
            </Text>
            {opt.description && (
              <Text
                style={[radioStyles.desc, { color: colors.mutedForeground }]}
              >
                {opt.description}
              </Text>
            )}
...
function PermissionCard({
  ...
  const { icon, color, text }: { icon: MIcon; color: string; text: string } =
    status === "granted"
      ? { icon: "check-circle", color: colors.success, text: "Granted" }
      : status === "denied"
        ? { icon: "cancel", color: colors.danger, text: "Denied" }
        : status === "checking"
          ? {
              icon: "schedule",
              color: colors.mutedForeground,
  ...
  return (
    ...
    >
      ...
      {status !== "granted" && (
        <TouchableOpacity
          style={[permStyles.btn, { backgroundColor: colors.primary }]}
          onPress={onRequest}
        >
          <Text style={permStyles.btnText}>
...
const LLM_PROVIDERS: {
  ...
  },
  {
    key: "ollama",
    label: "Ollama (Local)",
    description: "Llama 3.2, Mistral, Phi-3, etc.",
  },
  {
    key: "custom",
    label: "Custom API",
...
export default function SettingsScreen() {
  ...
  const indexMutation = useIndexDirectory({
    mutation: {
      onSuccess: (r) =>
        Alert.alert("Index Rebuilt", `${r.filesIndexed} files indexed`),
    },
  });
  ...
  const handleTestConnection = useCallback(async () => {
    if (!apiKeyInput.trim()) {
      Alert.alert("No API Key", "Enter an API key first");
      return;
    }
    ...
    try {
      ...
      if (
        ...
        url =
          (settings.llm.customEndpoint || "http://localhost:11434") +
          "/api/tags";
        headers = { "Content-Type": "application/json" };
        body = {};
      }
      const res = await fetch(url, {
        method: settings.llm.provider === "ollama" ? "GET" : "POST",
        headers,
  ...
  const handleLoadAiignore = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(
        `https://${domain}/api/files/content?path=.aiignore`,
      );
      if (res.ok) {
  ...
  const handleSaveAiignore = useCallback(async () => {
    try {
      ...
      Alert.alert("Error", "Failed to save .aiignore");
    }
  }, [aiignoreContent]);
    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      >
        ...
        <Section title="AI & LLM Configuration" icon="auto-awesome">
          <View
            style={[
              innerStyles.subheader,
              { borderBottomColor: colors.border },
            ]}
          >
            <Text
              style={[
                innerStyles.subheaderText,
                { color: colors.mutedForeground },
              ]}
            >
              Provider
          ...
          {/* API Key */}
          <View
            style={[innerStyles.inputGroup, { borderTopColor: colors.border }]}
          >
            <Text
              style={[
                innerStyles.inputLabel,
            ...
            <View style={innerStyles.apiKeyActions}>
              ...
              {apiKey && (
                <TouchableOpacity
                  ...
                  onPress={() =>
                    Alert.alert("Revoke Key", "Remove stored API key?", [
                      { text: "Cancel", style: "cancel" },
                      {
                        text: "Revoke",
                        style: "destructive",
          ...
          >
            ...
            <Slider
              ...
              onValueChange={(v) =>
                updateLLM({ temperature: parseFloat(v.toFixed(1)) })
              }
          ...
          >
            ...
            >
              Top P · {settings.llm.topP.toFixed(2)}
            </Text>
            <Slider
              style={innerStyles.slider}
              minimumValue={0}
              ...
              onValueChange={(v) =>
                updateLLM({ topP: parseFloat(v.toFixed(2)) })
              }
              minimumTrackTintColor={colors.primary}
              maximumTrackTintColor={colors.border}
              thumbTintColor={colors.primary}
        ...
        <Section title="Embedding & Search" icon="travel-explore">
          <SettingRow
            ...
            description={
              settings.embeddingModel === "builtin"
                ? "On-device TF-IDF (no API key required)"
                : settings.embeddingModel === "openai"
        ...
        <Section title="File Permissions" icon="security">
          ...
          >
            ...
            />
            <Text
              style={[innerStyles.infoText, { color: colors.mutedForeground }]}
            >
              All file operations run on the Node.js backend. The mobile app
              only makes API calls — it never touches your device storage
              directly, so no Android/iOS storage permissions are needed.
            </Text>
          </View>
        </Section>
        ...
        <Section title="Backup & Snapshots" icon="backup">
          ...
          >
            ...
            >
              Max local snapshots · {settings.maxLocalSnapshots}
            </Text>
            <Slider
              style={innerStyles.slider}
              minimumValue={1}
              ...
              onValueChange={(v) =>
                updateSettings({ maxLocalSnapshots: Math.round(v) })
              }
              minimumTrackTintColor={colors.primary}
              maximumTrackTintColor={colors.border}
              thumbTintColor={colors.primary}
        ...
        <Section title=".aiignore Rules" icon="rule">
          <View style={innerStyles.aiignoreHeader}>
            <Text
              style={[
                innerStyles.aiignoreInfo,
                { color: colors.mutedForeground },
              ]}
            >
              Files matching these patterns are excluded from indexing and all
              AI operations. Follows .gitignore syntax.
            </Text>
        ...
        <Section title="Transaction Behavior" icon="receipt-long">
          ...
          >
            ...
            >
              Transaction history · {settings.transactionHistorySize}
            </Text>
            <Slider
              style={innerStyles.slider}
              minimumValue={10}
              ...
              onValueChange={(v) =>
                updateSettings({ transactionHistorySize: Math.round(v) })
              }
              minimumTrackTintColor={colors.primary}
              maximumTrackTintColor={colors.border}
              thumbTintColor={colors.primary}
          ...
          <ToggleRow
            ...
            value={settings.enableDetailedLogs}
            onChange={(v) => updateSettings({ enableDetailedLogs: v })}
          />
          <SettingRow
            icon="delete-sweep"
            ...
            onPress={() =>
              Alert.alert(
                "Clear History",
                "This will permanently delete all transaction records.",
                [
                  { text: "Cancel", style: "cancel" },
                  {
                    text: "Clear",
        ...
        <Section title="Appearance & Behavior" icon="palette">
          ...
          <ToggleRow
            ...
            value={settings.notifyOnBackup}
            onChange={(v) => updateSettings({ notifyOnBackup: v })}
          />
          <ToggleRow
            icon="error-outline"
            label="Notify on Errors"
            value={settings.notifyOnErrors}
            onChange={(v) => updateSettings({ notifyOnErrors: v })}
          />
          ...
          <ToggleRow
            ...
            onChange={(v) => updateSettings({ verboseMode: v })}
            isLast
          />
        </Section>
        ...
        <Section title="About & Help" icon="info">
          <SettingRow
            icon="smartphone"
            label="App Version"
            description="AI File Manager 1.0.0 (build 1)"
            right={
              <Text
                style={[
                  innerStyles.valueText,
                  { color: colors.mutedForeground },
                ]}
              >
                1.0.0
          ...
          <SettingRow
            icon="code"
            label="Framework"
            description="Expo 54 · React Native 0.81 · Node 24"
            right={
              <Text
                style={[
                  innerStyles.valueText,
          ...
          <SettingRow
            ...
            onPress={() =>
              Linking.openURL(
                "mailto:support@example.com?subject=AI File Manager Bug",
              )
            }
          />
          <SettingRow
          ...
          <SettingRow
            ...
            onPress={() =>
              Alert.alert(
                "Privacy Policy",
                "Your data is stored locally and never transmitted without your consent.",
              )
            }
          />
          <SettingRow
            ...
            onPress={() =>
              Alert.alert(
                "Terms",
                "Use of this app is subject to our terms of service.",
              )
            }
          />
        </Section>
